...
function markerHtml(club, radius) {
  ...
  return `<div class="club-marker" style="width:${d}px;height:${d}px;background:${style.background};display:flex;align-items:center;justify-content:center;font-family:'IBM Plex Mono',monospace;font-weight:700;color:${style.text};font-size:${Math.max(6, radius * 0.55)}px;">${radius > 9 ? club.initials : ''}</div>`;
}
...
function showPanel(club) {
  ...
  header.innerHTML = `
    <div class="crest" style="background:${style.background};color:${style.text};">${club.initials}</div>
    <div>
