...
const POLITICAL_COLORS = {
  ...
  "Right-wing": "#2f6fce",
  "Far-right": "#6b4a2f",
  "Green/Environmentalist": "#2f9e44"
};

const APOLITICAL_COLOR = "#d8dee3";
...
const MARKER_TEXT_COLOR = {
  ...
  "Right-wing": "#f4f7ee",
  "Far-right": "#f4f7ee",
  "Green/Environmentalist": "#f4f7ee"
};

function markerColor(club) {
  return POLITICAL_COLORS[club.political_category] || APOLITICAL_COLOR;
}

function markerTextColor(club) {
  return MARKER_TEXT_COLOR[club.political_category] || "#0a0e14";
}
...
function markerHtml(club, radius) {
  const d = radius * 2;
  const color = markerColor(club);
  const textColor = markerTextColor(club);
  const ring = club.political_strong_evidence
    ? `box-shadow:0 0 0 ${Math.max(1.5, radius * 0.16)}px #d8b451, 0 0 6px rgba(216,180,81,0.6);`
    : '';
  return `<div class="club-marker" style="width:${d}px;height:${d}px;background:${color};${ring}display:flex;align-items:center;justify-content:center;font-family:'IBM Plex Mono',monospace;font-weight:700;color:${textColor};font-size:${Math.max(6, radius * 0.55)}px;">${radius > 9 ? club.initials : ''}</div>`;
}
...
function showPanel(club) {
  ...
  const color = markerColor(club);
  const ringStyle = club.political_strong_evidence
    ? `box-shadow:0 0 0 3px #d8b451, 0 0 10px rgba(216,180,81,0.5);`
    : '';

  const isApolitical = club.political_category === "Apolitical / no documented affiliation";
  const politicalBlock = isApolitical ? '' : `
    <div class="panel-record-label" style="margin-top:14px;">Supporter political leaning</div>
    <span class="panel-league-chip" style="background:${color};color:#0a0e14;border-color:${color};">${escapeHtml(club.political_category)}</span>
    <div class="panel-record" style="margin-top:6px;">${escapeHtml(club.political_explanation)}</div>
  `;

  content.innerHTML = `
    <div class="panel-header">
      <div class="crest" style="background:${color};color:${markerTextColor(club)};${ringStyle}">${club.initials}</div>
      <div>
